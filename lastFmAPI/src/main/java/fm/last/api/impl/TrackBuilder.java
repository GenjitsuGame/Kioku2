...
public class TrackBuilder extends XMLBuilder<Track> {
    ...
    public Track build(Node trackNode) {
        ...
        if (artistNode != null && artistNode.getChildNodes().getLength() > 1) {
        ...
        if(albumNode != null && albumNode.getChildNodes().getLength() > 1) {
            album = albumBuilder.build(albumNode);
            images = album.getImages();
        } else {
            album = new Album(artist.getName(), getText("album"), "", "", null);
        ...
        return new Track(id, name, mbid, url, duration, streamable, listeners, playcount, artist, album, images, date, nowPlaying);
    }
}